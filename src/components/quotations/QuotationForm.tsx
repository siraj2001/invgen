...
export function QuotationForm() {
  ...
  const fetchInitialData = async () => {
    ...
    if (isEditing && id) {
      ...
      setValue('quotation_number', `QUO-SKE${yyyy}${mm}${dd}-${seq}`);
    }
    setLoading(false);
  };
  ...
  const onSubmit = async (data: Quotation) => {
    ...
    try {
      ...
      if (isEditing) {
        const { error } = await supabase.from('quotations').update(quotationData).eq('id', id);
        if (error) throw error;
        await supabase.from('quotation_items').delete().eq('quotation_id', id);
      } else {
        const { id: _, ...insertData } = quotationData as any;
        const { data: newQuotation, error } = await supabase.from('quotations').insert([insertData]).select().single();
        if (error) throw error;
        quotationId = newQuotation.id;
      }

      if (items && items.length > 0 && quotationId) {
        const itemsToInsert = items.map(item => {
          const { id: _id, ...rest } = item as any;
          return { ...rest, quotation_id: quotationId };
        });
    ...
    <div className="max-w-5xl mx-auto py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <button onClick={() => navigate('/quotations')} className="mr-4 text-zinc-500 hover:text-zinc-900 transition-colors">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500 mt-1">Fill in the details below to create your quotation.</p>
          </div>
        </div>
        <button
      ...
      <form className="card p-8">
        <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-700">Quotation Date</label>
            <input type="date" {...register('date', { required: true })} className="input-field mt-1" />
          </div>
          <div>
            <label className="block text-sm font-medium text-zinc-700">Valid Until</label>
            <input type="date" {...register('valid_until', { required: true })} className="input-field mt-1" />
          </div>
        </div>
                  ...
                  <div className="space-y-4 pr-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="number" step="0.01" {...register(`items.${index}.quantity`, { required: true })} className="input-field" />
                      </div>
                      <div>
                ...
                {fields.map((field, index) => (
                  <tr key={field.id}>
                    ...
                    <td className="px-3 py-4 align-top">
                      <input type="text" {...register(`items.${index}.hsn_sac`)} className="input-field text-center" placeholder="HSN/SAC" />
                    </td>
                    <td className="px-3 py-4 align-top">
                      <input type="number" step="0.01" {...register(`items.${index}.quantity`, { required: true })} className="input-field text-right" />
                    </td>
                    <td className="px-3 py-4 align-top">
        ...
        <div className="mt-10 border-t pt-8 flex flex-col md:flex-row justify-between">
          <div className="md:w-1/2 pr-8 space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">Scope of Work / Terms</label>
              <textarea {...register('scope_of_work')} rows={4} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm p-2 border" placeholder="Scope of work details..." />
            </div>
            <div>
